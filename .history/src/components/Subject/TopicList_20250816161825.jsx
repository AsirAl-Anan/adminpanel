  ...
  const handleEditTopic = async () => {
    ...
    try {
      ...
      if (newTopic.englishName !== originalTopic.englishName) 
        changedFields.englishName = newTopic.englishName;
      if (newTopic.banglaName !== originalTopic.banglaName) 
        changedFields.banglaName = newTopic.banglaName;
      if (newTopic.topicCode !== originalTopic.topicCode) 
        changedFields.topicCode = newTopic.topicCode;
      if (newTopic.englishDescription !== originalTopic.englishDescription) 
        changedFields.englishDescription = newTopic.englishDescription;
      if (newTopic.banglaDescription !== originalTopic.banglaDescription) 
        changedFields.banglaDescription = newTopic.banglaDescription;
      
      // Handle aliases
      if (JSON.stringify(newTopic.aliases) !== JSON.stringify(originalTopic.aliases)) 
        changedFields.aliases = newTopic.aliases;
      
      // Handle formulas
      if (JSON.stringify(newTopic.formulas) !== JSON.stringify(originalTopic.formulas)) 
        changedFields.formulas = newTopic.formulas;
      ...
      if (newImages.length > 0) {
        ...
        changedFields.imageTitles = newImages.map(img => img.title);
      }
      
      // Append changed fields to formData
      Object.keys(changedFields).forEach(key => {
        if (key === 'aliases' || key === 'formulas') {
