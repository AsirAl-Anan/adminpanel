...
const handleEditTopic = async () => {
  ...
  try {
    ...
    if (newImages.length > 0) {
      ...
      formData.append('imageTitles', JSON.stringify(newImages.map(img => img.title)));
      hasChanges = true;
    }
    
    // Handle existing image title updates
    const existingImages = newTopic.images.filter(img => !img.file && img.title !== undefined);
    if (existingImages.length > 0) {
      const titles = existingImages.map(img => img.title);
      const indices = existingImages.map(img => newTopic.images.indexOf(img));
      formData.append('existingImageTitles', JSON.stringify(titles));
      formData.append('existingImageIndices', JSON.stringify(indices));
      hasChanges = true;
    }
    ...
    if (!hasChanges && newImages.length === 0 && existingImages.length === 0) {
