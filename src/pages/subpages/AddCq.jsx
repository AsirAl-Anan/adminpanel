...
import { Upload, RotateCcw, BookOpen, Image, Plus, X, Loader2, CheckCircle, AlertCircle, ArrowLeft } from 'lucide-react';
import LatexRenderer from './LatexRenderer';
import 'katex/dist/katex.min.css';
import axios from '../../config/axios.js';
import { useNavigate } from 'react-router-dom';
import { showSuccessToast, showErrorToast, showInfoToast, showWarningToast } from '../../components/toast/Toast.jsx';
    ...
    <div className="space-y-1.5">
      ...
      {!imagePreview ? (
        ...
        <div className="relative w-40 h-40">
            <img src={imagePreview} alt="Preview" className="object-cover w-full h-full rounded-md border border-slate-200" />
            <button type="button" className="absolute top-1.5 right-1.5 bg-white rounded-full p-1 shadow-md text-slate-600 hover:text-red-500 transition-colors" onClick={clearImage}>
                <X size={16} />
            </button>
        </div>
      )}
...
const ImageUploadModal = ({ isOpen, onClose, onSubmit, isLoading, title, buttonText, maxImages }) => {
    ...
    const handleImageChange = (e) => {
      ...
      if (files.length + selectedImages.length > maxImages) {
        showErrorToast(`You can only upload up to ${maxImages} images.`); return;
      }
    ...
    const handleSubmit = () => {
      if (selectedImages.length === 0) { showErrorToast("Please select at least one image."); return; }
      onSubmit(selectedImages);
    };
  
    const handleClose = () => {
      imagePreviews.forEach((url) => URL.revokeObjectURL(url));
      setSelectedImages([]); setImagePreviews([]); onClose();
    };
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-60 backdrop-blur-sm">
        <div className="relative w-full max-w-2xl overflow-hidden bg-white rounded-xl shadow-2xl">
          <div className="flex items-center justify-between p-5 border-b"><h2 className="text-xl font-semibold text-gray-800">{title}</h2><button onClick={handleClose} disabled={isLoading} className="p-1 text-gray-500 rounded-full hover:bg-gray-100 disabled:opacity-50"><X size={20} /></button></div>
          <div className="p-6">
            <div className="flex flex-col items-center justify-center p-6 mb-4 border-2 border-dashed rounded-lg bg-slate-50">
                <input type="file" ref={fileInputRef} onChange={handleImageChange} accept="image/*" multiple className="hidden" disabled={isLoading || selectedImages.length >= maxImages} />
                <Image className="w-10 h-10 mb-3 text-gray-400" />
                <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isLoading || selectedImages.length >= maxImages} className="flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
                    <Plus size={16} className="mr-2" /><span>Add Images</span>
                </button>
                <p className="mt-2 text-xs text-gray-500">Max {maxImages} images. ({selectedImages.length}/{maxImages} selected)</p>
            </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
                    {imagePreviews.map((preview, index) => (
                        <div key={index} className="relative group">
                            ...
                            <button type="button" onClick={() => removeImage(index)} disabled={isLoading} className="absolute -top-2 -right-2 p-1 text-white bg-red-500 rounded-full opacity-0 group-hover:opacity-100 disabled:opacity-50 transition-opacity">
                                <X size={14} />
                            </button>
                        </div>
                    ))}
          ...
          <div className="flex items-center justify-end p-4 space-x-3 border-t bg-gray-50">
              <button type="button" onClick={handleClose} disabled={isLoading} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border rounded-md hover:bg-gray-50 disabled:opacity-50">Cancel</button>
              <button type="button" onClick={handleSubmit} disabled={isLoading || selectedImages.length === 0} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed">
                  {isLoading ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Uploading...</>) : (buttonText)}
              </button>
          </div>
        </div>
      </div>
                    ...
                    return (
                        <li key={stepName} className="relative">
                            {stepIdx !== steps.length - 1 && (
                                <div className={`absolute left-4 top-4 -ml-px mt-0.5 h-full w-0.5 ${isCompleted ? 'bg-blue-600' : 'bg-slate-300'}`} />
                            )}
                            <div className="relative flex items-start group">
                                ...
                                <span className="ml-4 flex min-w-0 flex-col mt-1">
                                    <span className={`text-sm font-medium ${isCurrent ? 'text-blue-600' : 'text-slate-500'}`}>{stepName}</span>
                                </span>
                            </div>
                        </li>
                    )
...
const QuestionEntryStep = ({ version, data, handlers, errors, dynamicTopics, isMathSubject, onAiClick }) => {
    ...
    return (
        <div className="space-y-6">
            <button type="button" onClick={onAiClick} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                <Image size={16} className="mr-2" /> Add Question from Image
            </button>
            
            <div>
                <FormField label={`${versionTitle} Stem`} isRequired>
                    <StyledTextarea value={data.stem} onChange={(e) => onInputChange('stem', e.target.value)} placeholder={`Enter the main question in ${versionTitle}...`} rows="6" error={errors.stem} />
                </FormField>
            ...
            <ImageUploader imageFile={data.stemImageFile} imagePreview={data.stemImagePreview} onImageChange={(file, preview) => onImageChange('stem', file, preview)} label="Stem Image (Optional)" />
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-4">
                        ...
                        <FormField label="Question Type for C" isRequired error={errors.cQuestionType}>
                            <StyledSelect
                                ...
                                disabled={!data.cTopic || !data.cTopic.value.questionTypes || data.cTopic.value.questionTypes.length === 0}
                                error={errors.cQuestionType}
                            >
                                <option value="">Select Question Type</option>
                                {data.cTopic?.value?.questionTypes?.map(qType => (
                                    <option key={qType._id} value={qType.english}>{qType.english}</option>
                                ))}
                            </StyledSelect>
                        </FormField>
                    </div>
                        ...
                        <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-4">
                            ...
                            <FormField label="Question Type for D" isRequired={!isMathSubject} error={errors.dQuestionType}>
                                <StyledSelect
                                    ...
                                    disabled={!data.dTopic || !data.dTopic.value.questionTypes || data.dTopic.value.questionTypes.length === 0}
...
const AnswerEntryStep = ({ data, handlers, errors, isMathSubject, onAiClick }) => {
    ...
    return (
        <div className="space-y-6">
            <button type="button" onClick={onAiClick} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                <Image size={16} className="mr-2" /> Add Answers from Image
            </button>
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
...
const createInitialVersionState = () => ({
    stem: '', stemImageFile: null, stemImagePreview: null,
    a: '', aAnswer: '',
    b: '', bAnswer: '',
    c: '', cAnswer: '', cTopic: null, cQuestionType: '', cAnswerImageFile: null, cAnswerImagePreview: null, cType: 'Application',
    d: '', dAnswer: '', dTopic: null, dQuestionType: '', dAnswerImageFile: null, dAnswerImagePreview: null, dType: 'Higher Order'
});
...
const AddCreativeQuestionPage = () => {
    ...
    const [formData, setFormData] = useState({
        group: '', board: '', institution: { name: '', examType: '' },
        year: new Date().getFullYear(), subject: '', level: '', chapter: null,
    ...
    useEffect(() => {
        const fetchSubjects = async () => {
          if (formData.group && formData.level) {
            setIsSubjectsLoading(true); setSubjectsError(null); setDynamicSubjects([]);
            setDynamicChapters([]); setDynamicTopics([]);
            ...
            } catch (err) { setSubjectsError(err.message || 'An error occurred.'); } 
    ...
    useEffect(() => {
        if (formData.chapter) {
            const topics = formData.chapter?.topics.map(t => ({ value: t, label: `${t.englishName} (${t.banglaName})` })) || [];
    ...
    const isMathSubject = useMemo(() => {
        ...
        return ["General Mathematics", "Higher Mathematics", "Higher Mathematics 1st Paper", "Higher Mathematics 2nd Paper"].includes(subject?.englishName);
    }, [formData.subject, dynamicSubjects]);
    ...
    const handleVersionedInputChange = (version, field, value) => setFormData(prev => ({ ...prev, [version]: { ...prev[version], [field]: value } }));
    const handleVersionedImageChange = (version, fieldPrefix, file, preview) => setFormData(prev => ({ ...prev, [version]: { ...prev[version], [`${fieldPrefix}ImageFile`]: file, [`${fieldPrefix}ImagePreview`]: preview }}));
    ...
    const handleAiResponse = (responseData, version) => {
        setFormData(prev => ({ ...prev, [version]: { ...prev[version], stem: responseData.stem || prev[version].stem, a: responseData.a || prev[version].a, aAnswer: responseData.aAnswer || prev[version].aAnswer, b: responseData.b || prev[version].b, bAnswer: responseData.bAnswer || prev[version].bAnswer, c: responseData.c || prev[version].c, cAnswer: responseData.cAnswer || prev[version].cAnswer, d: responseData.d || prev[version].d, dAnswer: responseData.dAnswer || prev[version].dAnswer, }}));
    };
    const handleImageUploadSubmit = async (images) => {
        setIsImageUploadLoading(true);
        try { const formDataToSend = new FormData(); images.forEach((image) => formDataToSend.append('qb', image)); const response = await axios.post('/ai/extract-cq', formDataToSend); handleAiResponse(response.data.data[0], 'english'); handleAiResponse(response.data.data[1], 'bangla'); setIsImageUploadModalOpen(false); showSuccessToast("Questions generated successfully!"); } catch (error) { showErrorToast('Failed to generate questions.'); } finally { setIsImageUploadLoading(false); }
    };
    const handleAnswerUploadSubmit = async (images) => {
        setIsAnswerUploadLoading(true);
        try { const formDataToSend = new FormData(); images.forEach((image) => formDataToSend.append('qb', image)); const response = await axios.post('/ai/extract-cq-answers', formDataToSend); handleAiResponse(response.data.data[0], 'english'); handleAiResponse(response.data.data[1], 'bangla'); setIsAnswerUploadModalOpen(false); showSuccessToast("Answers generated successfully!"); } catch (error) { showErrorToast('Failed to generate answers.'); } finally { setIsAnswerUploadLoading(false); }
    };
    ...
    const buildSubmissionPayload = (versionName, linkingId) => {
        ...
        if (formData.chapter) {
            const chapterPayload = { chapterId: formData.chapter._id, englishName: formData.chapter.englishName, banglaName: formData.chapter.banglaName };
        ...
        if (versionData.d && versionData.d.trim() !== '') {
            ...
            if (versionData.dTopic && versionData.dTopic.value) {
                const dTopicPayload = { topicId: versionData.dTopic.value._id, englishName: versionData.dTopic.value.englishName, banglaName: versionData.dTopic.value.banglaName };
    ...
    const resetForm = () => {
        setFormData({ 
            group: '', board: '', institution: { name: '', examType: '' }, 
            year: new Date().getFullYear(), subject: '', level: '', chapter: null, 
            english: createInitialVersionState(), bangla: createInitialVersionState(), 
        });
        setStep(0); setErrors({}); setDynamicSubjects([]); setDynamicChapters([]); setDynamicTopics([]); setSubmissionSuccess(false); setSubmissionError('');
    };

    const formTitles = ["Basic Details", "Subject & Context", "English Question", "Bangla Question", "English Answer", "Bangla Answer"];
    
    return (
        <div className="min-h-screen bg-slate-100">
            <ImageUploadModal isOpen={isImageUploadModalOpen} onClose={() => setIsImageUploadModalOpen(false)} onSubmit={handleImageUploadSubmit} isLoading={isImageUploadLoading} title="Upload Question Images" buttonText="Generate Question" maxImages={6} />
            <AnswerUploadModal isOpen={isAnswerUploadModalOpen} onClose={() => setIsAnswerUploadModalOpen(false)} onSubmit={handleAnswerUploadSubmit} isLoading={isAnswerUploadLoading} title="Upload Answer Images" buttonText="Generate Answers" maxImages={6} />
            
            <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                <div className="mb-8">
                    <button onClick={() => navigate(-1)} className='inline-flex items-center text-sm font-medium text-slate-600 hover:text-blue-600'>
                    ...
                    <main className="lg:col-span-9 xl:col-span-10">
                        <form onSubmit={handleSubmit}>
                            <FormCard title={formTitles[step]}>
                                {submissionSuccess && <div className="p-4 mb-4 text-green-800 bg-green-100 rounded flex items-center"><CheckCircle className="mr-2" />Success! Question created.</div>}
                                {submissionError && <div className="p-4 mb-4 text-red-800 bg-red-100 rounded flex items-center"><AlertCircle className="mr-2" />Error! {submissionError}</div>}
                                
                                {step === 0 && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <FormField label="Level" isRequired error={errors.level}>
                                            <StyledSelect value={formData.level} onChange={(e) => handleRootChange('level', e.target.value)} error={errors.level}>
                                                <option value="">Select Level</option><option value="SSC">SSC</option><option value="HSC">HSC</option>
                                            </StyledSelect>
                                        </FormField>
                                        <FormField label="Group" isRequired error={errors.group}>
                                            <StyledSelect value={formData.group} onChange={(e) => handleRootChange('group', e.target.value)} error={errors.group}>
                                                <option value="">Select Group</option><option value="science">Science</option><option value="arts">Arts</option><option value="commerce">Commerce</option>
                                            </StyledSelect>
                                        </FormField>
                                ...
                                {step === 1 && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <FormField label="Subject" isRequired error={errors.subject}>
                                            <StyledSelect value={formData.subject} onChange={(e) => handleRootChange('subject', e.target.value)} error={errors.subject}><option value="">Select Subject</option>{dynamicSubjects.map(s => <option key={s._id} value={s._id}>{s.englishName}</option>)}</StyledSelect>
                                        </FormField>
                                        <FormField label="Chapter" isRequired error={errors.chapter}>
                                            <StyledSelect value={formData.chapter?._id || ''} onChange={(e) => handleRootChange('chapter', dynamicChapters.find(c => c._id === e.target.value))} disabled={!formData.subject} error={errors.chapter}><option value="">Select Chapter</option>{dynamicChapters.map(c => <option key={c._id} value={c._id}>{c.englishName}</option>)}</StyledSelect>
                                        </FormField>
                                        <FormField label="Board (Optional)" error={errors.board}>
                                            <StyledSelect value={formData.board} onChange={(e) => handleRootChange('board', e.target.value)} error={errors.board}><option value="">Select Board</option>{['Dhaka', 'Chittagong', 'Rajshahi', 'Sylhet', 'Comilla', 'Jessore', 'Dinajpur', 'Mymensingh', 'Madrasah', 'Barishal'].map(b => <option key={b} value={b}>{b}</option>)}</StyledSelect>
                                        </FormField>
                                        <FormField label="Year" isRequired error={errors.year}>
                                            <StyledInput type="number" min="2000" max="2099" value={formData.year} onChange={(e) => handleRootChange('year', e.target.value)} error={errors.year} />
                                        </FormField>
                                        ...
                                        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <FormField label="Institution Name (Optional)">
                                                <StyledInput 
                                                    ...
                                                    onChange={(e) => handleRootChange('institution', { ...formData.institution, name: e.target.value })} 
                                            ...
                                            <FormField label="Exam Type (Optional)">
                                                <StyledSelect 
                                                    ...
                                                    onChange={(e) => handleRootChange('institution', { ...formData.institution, examType: e.target.value })}
                                ...
                                {step === 2 && <QuestionEntryStep version="english" data={formData.english} errors={errors.english || {}} dynamicTopics={dynamicTopics} isMathSubject={isMathSubject} handlers={{ onInputChange: (f, v) => handleVersionedInputChange('english', f, v), onImageChange: (f, file, p) => handleVersionedImageChange('english', f, file, p), onTopicChange: (f, v) => handleVersionedTopicChange('english', f, v) }} onAiClick={() => setIsImageUploadModalOpen(true)} />}
                                {step === 3 && <QuestionEntryStep version="bangla" data={formData.bangla} errors={errors.bangla || {}} dynamicTopics={dynamicTopics} isMathSubject={isMathSubject} handlers={{ onInputChange: (f, v) => handleVersionedInputChange('bangla', f, v), onImageChange: (f, file, p) => handleVersionedImageChange('bangla', f, file, p), onTopicChange: (f, v) => handleVersionedTopicChange('bangla', f, v) }} onAiClick={() => setIsImageUploadModalOpen(true)} />}
                                {step === 4 && <AnswerEntryStep version="english" data={formData.english} errors={errors.english || {}} isMathSubject={isMathSubject} handlers={{ onInputChange: (f, v) => handleVersionedInputChange('english', f, v), onImageChange: (f, file, p) => handleVersionedImageChange('english', f, file, p) }} onAiClick={() => setIsAnswerUploadModalOpen(true)} />}
                                {step === 5 && <AnswerEntryStep version="bangla" data={formData.bangla} errors={errors.bangla || {}} isMathSubject={isMathSubject} handlers={{ onInputChange: (f, v) => handleVersionedInputChange('bangla', f, v), onImageChange: (f, file, p) => handleVersionedImageChange('bangla', f, file, p) }} onAiClick={() => setIsAnswerUploadModalOpen(true)} />}

                                <div className="flex items-center justify-between pt-6 border-t border-slate-200 mt-6">
                                    <button type="button" onClick={resetForm} className="flex items-center px-4 py-2 text-sm font-medium text-slate-600 bg-transparent rounded-md hover:bg-slate-100 hover:text-slate-800 disabled:opacity-50">
                                        <RotateCcw className="w-4 h-4 mr-2" /> Reset Form
                                    </button>
                                    <div className="flex items-center space-x-3">
                                        {step > 0 && 
                                            <button type="button" onClick={handlePrev} className="px-5 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                                Previous
                                            </button>
                                        }
                                        {step === 5 ? (
                                            <button type="submit" disabled={isSubmitting} className="inline-flex justify-center px-5 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-blue-400 disabled:cursor-not-allowed">
                                                {isSubmitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Submitting...</> : 'Submit Question'}
                                            </button>
                                        ) : (
                                            <button type="button" onClick={handleNext} className="inline-flex justify-center px-5 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                                Next
                                            </button>
                                        )}
                                    </div>
