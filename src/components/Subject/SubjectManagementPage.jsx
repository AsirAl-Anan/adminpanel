...
const SubjectManagementPage = () => {
  ...
  const fetchSubjectData = async () => {
    ...
    try {
      ...
      console.error(err)
      showErrorToast("Failed to load subject data")
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="bg-background min-h-screen">
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          {subject && (
            <p className="text-muted-foreground mt-1">Edit details, chapters, and topics for "{subject.name?.en}"</p>
          )}
        </div>
        ...
        {/* Tab Content */}
        <div>
          {activeTab === "details" && subject && <SubjectDetails subject={subject} onSave={fetchSubjectData} />}
          {activeTab === "chapters" && subject && <ChapterList subject={subject} onUpdate={fetchSubjectData} />}
        </div>
      </div>
    </div>
  )
